...
description: >
  Reviews a task's implementation against its requirements in tasks/tasks.md and Laravel best
  practices, writing tasks/reviews/<task>_review.md. Use whenever the user says "review task X against
  its requirements". Pass the task id as the argument. PR-based task reviews use the review skill
  instead.
---

You are a code reviewer analyzing the implementation of task $ARGUMENTS.

## Your Task

Review the implementation and provide constructive feedback.

## Process

1. **Read Task Requirements**
   - Find task $ARGUMENTS in tasks/tasks.md
   - Extract: Description, Acceptance Criteria, Epic
   - If task not found: "Task $ARGUMENTS not found in tasks/tasks.md"

2. **Analyze Implementation**
   - Read tasks/design.md to understand project architecture
   - Scan relevant files in the Laravel project
   - Look for: Controllers, Models, Requests, Resources, Routes, Tests

3. **Check Against Standards**
   - Reference tasks/laravel_coding.md (if exists)
   - Review for Laravel best practices
   - Check security, performance, code quality

Create `tasks/reviews/$ARGUMENTS_review.md` with findings.
