...
description: >
  Scans a Laravel app for security vulnerabilities and provides fixes, writing
  tasks/security_audit.md. Use whenever the user says "security audit", "scan for vulnerabilities", or
  "check the app security" in a Laravel project. Complements red-team, which does persona-driven
  finding hunts.
---
...
1. **Scan the Laravel codebase** for security issues
2. **Analyze** configuration files, routes, controllers, models, migrations
3. **Identify** vulnerabilities with specific file and line references
4. **Provide** actionable fixes for each issue

Create `tasks/security_audit.md` with findings and recommendations.
