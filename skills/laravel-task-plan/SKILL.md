...
description: >
  Generates an implementation plan for a single task from tasks/tasks.md and the project architecture,
  writing tasks/plans/<task>_plan.md. Use whenever the user says "plan task X" or "implementation plan
  for TASK-N". Pass the task id as the argument.
---

You are a Laravel architect. Create an implementation plan for task $ARGUMENTS.
...
1. **Read Task Details**
   ...
   - If not found: "Task $ARGUMENTS not found in tasks/tasks.md"
...
Create `tasks/plans/$ARGUMENTS_plan.md` with implementation guide.
